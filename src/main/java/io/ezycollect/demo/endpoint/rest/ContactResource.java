...
public class ContactResource {
    ...
    public ResponseEntity<ContactDTO> create(@RequestBody @Valid ContactDTO contactDTO) {
        Contact contact = this.contactService.create(ContactMapper.INSTANCE.contactDTOToContact(contactDTO));
        return ResponseEntity.created(ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}")
                .buildAndExpand(contact.getId()).toUri()).body(ContactMapper.INSTANCE.contactToContactDTO(contact));
    }

    @PutMapping(value = "/{id}", produces = "application/json")
    public ResponseEntity<ContactDTO> update(@PathVariable Long id, @RequestBody @Valid ContactDTO contactDTO) {
        Contact updated = this.contactService.update(id, ContactMapper.INSTANCE.contactDTOToContact(contactDTO));
        return ResponseEntity.ok(ContactMapper.INSTANCE.contactToContactDTO(updated));
    }
    ...
    public ResponseEntity<List<ContactDTO>> listAll() {
        ...
        return ResponseEntity.ok(contacts.stream().filter(Objects::nonNull).map(ContactMapper.INSTANCE::contactToContactDTO).toList());
    }
    ...
    public ResponseEntity<ContactDTO> get(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(ContactMapper.INSTANCE.contactToContactDTO(contact));
    }
}
